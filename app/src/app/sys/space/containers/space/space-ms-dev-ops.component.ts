...
import {MsDevOpsDiff} from 'term-web/integration/_lib/msDevOps/msDevOps';
...
/**
 * Space → MS DevOps (Azure) git sync — parallel to {@link SpaceGithubComponent}.
 * Gated behind environment.msDevOpsEnabled until termx-server exposes /spaces/{id}/msdevops/*.
 */
@Component({
    templateUrl: './space-ms-dev-ops.component.html',
    styles: [`
    .status-row {
      display: flex;
...
export class SpaceMsDevOpsComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this.spaceMsDevOpsService.authenticate(id, `${window.location.origin + environment.baseHref}${window.location.pathname}`).subscribe(r => {
  ...
  private loadGitStatus(id: number): Observable<any> {
    return this.spaceMsDevOpsService.status(id).pipe(tap(r => {
      this.status = {
  ...
  public showDiff(file: string): void {
    this.diff = {};
    this.spaceMsDevOpsService.diff(this.space.id, file).subscribe(r => this.diff = r);
  }

  public pull(): void {
    const files = this.allSelected(this.selection) ? null : this.status.changed.filter(c => this.selected(c)).map(c => c.f);
    const req$ = this.spaceMsDevOpsService.pull(this.space.id, files).pipe(mergeMap(() => this.loadGitStatus(this.space.id)));
    this.loader.wrap('pull', req$).subscribe();
  }

  public push(): void {
    const files = this.allSelected(this.selection) ? null : this.status.changed.filter(c => this.selected(c)).map(c => c.f);
    const req$ = this.spaceMsDevOpsService.push(this.space.id, this.commit.message, files).pipe(
      delay(1000), // give the remote a bit of time to actually update
  ...
  public selectAll(selected: boolean, files?: string): void {
    const prefixes = !files ? null : files.split(',').map(f => {
      const d = f.split('|');
      return this.space.integration.msDevops.dirs[d[0]] + '/' + d[1];
    });
    this.status.changed

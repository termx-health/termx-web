...
export class ValueSetRuleEditComponent implements OnInit {
  ...
  public valueSetVersionCode?: string | null;
  ...
  public ngOnInit(): void {
    ...
    this.valueSetVersionCode = this.route.snapshot.paramMap.get('versionCode');
    ...
    if (this.mode === 'edit') {
      ...
      this.rule = this.writeRule(new ValueSetVersionRule());
    }
  }

  private loadRule(valueSetId: string, ruleId: number): void {
    this.loading['init'] = true;
    this.valueSetService.loadRule(valueSetId, ruleId).subscribe(rule => this.rule = this.writeRule(rule)).add(() => this.loading['init'] = false);
  }
  ...
  private writeRule(rule: ValueSetVersionRule): ValueSetVersionRule {
    rule.concepts ??= [];
    rule.filters ??= [];
    rule.type ??= 'include';

...
export class CodeSystemVersionInfoWidgetComponent implements OnChanges {
  ...
  private saveFile(fhirCs: any, format: string): void {
    ...
    if (format === 'fsh r4') {
      this.fhirToFsh(fhirCs.id, json, '4.3.0');
    }
    if (format === 'fsh r5') {
      this.fhirToFsh(fhirCs.id, json, '5.0.0');
    }
  }

  private fhirToFsh(codeSystemId: string, json: string, version: string): void {
    this.chefService.fhirToFsh({fhir: [json]}, version).subscribe(r => {
      ...
      saveAs(new Blob([fsh], {type: 'application/fsh'}), `CS-${codeSystemId}.fsh`);
    });    
  }
